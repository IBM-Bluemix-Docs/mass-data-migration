...
## Qu'est-ce que l'interface de fichiers ?
{: faq}

Par défaut, des partages NFS (Network File System) et SMB (Server Message Block) sont activés sur le périphérique Mass Data Migration.
...
Tout d'abord, déverrouillez le pool de chiffrement. Ensuite, montez le partage sur le serveur contenant les données que vous avez l'intention de migrer. Commencez à copier vos fichiers de données dans le partage.
...
## Comment le processus {{site.data.keyword.cloud_notm}} Mass Data Migration maintient-il le caractère unique des noms d'objet ?
{: faq}
