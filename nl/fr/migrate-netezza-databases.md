...
# Migration des bases de données PureData System for Analytics vers {{site.data.keyword.dashdbshort_notm}}
{: #migrate-netezza-databases}

{{site.data.keyword.mdms_full}} peut être utilisé pour migrer de grosses bases de données IBM PureData™ System for Analytics (optimisées par la technologie Netezza®) vers {{site.data.keyword.dashdbshort}}. Vous pouvez utiliser ce document comme référence pour les outils qui permettent de déterminer la quantité de données à transférer et les méthodes d'exportation.
{: shortdesc}
...
- Utilisez l'instruction `CREATE EXTERNAL TABLE`.
   ...
   - Fournissez à l'équipe {{site.data.keyword.dashdbshort_notm}} la clause `USING` utilisée pour l'exportation à des fins de réutilisation pendant le processus `LOAD`.
...
Pour plus d'informations sur PureData System for Analytics, voir [IBM Netezza database user documentation ![Icône de lien externe](../../icons/launch-glyph.svg "Icône de lien externe")](https://www.ibm.com/support/knowledgecenter/en/SSULQD_7.2.1/com.ibm.nz.dbu.doc/c_dbuser_plg_overview.html){:new_window}.
