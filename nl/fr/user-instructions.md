...
Vous devez établir deux connexions Ethernet. La première pour la gestion du périphérique via un navigateur, et la deuxième pour le transfert de données sur le même sous-réseau que celui où résident les données source. {{site.data.keyword.cloud}} fournit deux modèles de périphérique MDMS. Un modèle prend en charge uniquement la connectivité RJ45.  SFP+ et RJ45 cuivre sont pris en charge par l'autre modèle. Suivez les instructions adaptées au modèle du périphérique MDMS dont vous disposez. 
...
Les ports proviennent du périphérique car les câbles RJ45 et CAT6A sont fournis. Des adaptateurs SFP+ cuivre sont fournis pour la conversion à partir de RJ45. Les adaptateurs sont compatibles avec tous les fabricants d'interrupteur. Ces adaptateurs se trouvent dans une pochette dans la section inférieure du couvercle du couvercle du carton d'expédition.
...
Les ports proviennent du périphérique en tant que SFP+ et RJ45 cuivre. Les câbles CAT6A et SFP+ cuivre sont fournis. 
...
>*Remarque** : il n'est pas nécessaire de configurer/d'utiliser les deux ports si l'un d'eux est accessible via un navigateur Web. 
...
2.	Déterminez l'emplacement le plus approprié pour l'installation du périphérique, proche d'une prise secteur et d'une prise Ethernet et avec une circulation réduite. 
