...
영업일 기준 처음 10일 동안은 무료로 디바이스를 현장에 보관할 수 있습니다. 이 시간 범위에는 디바이스가 배송된 날 또는 디바이스를 받은 날은 포함되지 않습니다. 데이터 삽입을 완료하는 데 시간이 더 필요하면 그 이후에 1일당 USD 30을 지불하고 사용 기간을 연장할 수 있습니다(미국 및 EU 지역에 적용됨).

<hr/>

## **대량 데이터 마이그레이션에서는 어떤 네트워크 인터페이스를 지원합니까?**

대량 데이터 마이그레이션 디바이스에는 RJ45(CAT6a) 및 SFP+ 구리선 네트워크 포트를 포함하는 10Gbps 네트워크 인터페이스가 있습니다. RJ45 - SFP+ 변환기가 포함됩니다. 10Gbps 인터페이스에는 Jumbo Frame이 사용으로 설정되어 있습니다. 
...
## **{{site.data.keyword.cloud_notm}}에 데이터를 가져오는 데는 비용이 얼마나 듭니까?**

{{site.data.keyword.cloud_notm}}에 전송되는 데이터에는 요금이 부과되지 않습니다.
