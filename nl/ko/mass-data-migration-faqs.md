...
여러 디바이스를 병렬 또는 직렬로 사용하여 한 번의 마이그레이션으로 모든 데이터를 이동하거나 하나의 디바이스를 사용하여 마이그레이션을 반복 수행하십시오. 예를 들어, 9개 디바이스를 병렬로 사용하여 1PB의 데이터를 전송하거나, 하나의 디바이스를 사용하여 개별 마이그레이션을 9번 수행하십시오.

## Q6. 데이터를 전송하는 데 얼마나 걸립니까? 
고객이 대량 데이터 마이그레이션 요청을 제출한 시점부터 {{site.data.keyword.cos_full}} 버킷에서 데이터에 액세스할 수 있을 때까지 소요되는 시간은 7일 정도입니다.  

## Q7. 대량 데이터 마이그레이션 디바이스를 얼마나 보유할 수 있습니까?  
디바이스는 처음 10일 동안 무료로 현장에 보관할 수 있습니다. 디바이스를 배송한 날 또는 받은 날은 포함되지 않습니다. 데이터 삽입을 완료하는 데 추가 시간이 필요한 경우 이후로 1일당 USD 30을 지불하고 사용 기간을 연장할 수 있습니다(미국 및 EU 지역에 해당). 
...
## Q9. 대량 데이터 마이그레이션 기본 발송 옵션은 무엇입니까? 
...
대량 데이터 마이그레이션은 현재 미국 및 EU에서만 사용할 수 있습니다. 각각 서비스의 US 표준 Cross Region 또는 EU Cross Region 티어의 {{site.data.keyword.cos_full}}에 모든 데이터가 마이그레이션됩니다. 디바이스는 한 지역에서 발송하고 다른 지역으로 리턴할 수 없습니다.
