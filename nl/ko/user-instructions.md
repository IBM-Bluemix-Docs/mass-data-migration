...
2. 연결 링크 LED 옆에 있는 시스템 켜기 / 끄기 단추를 사용하십시오.
...
- Eth1(`1GbE-B`)은 일반적으로 디바이스 관리에 사용되며, 따라서 게이트웨이가 IP 주소 구성에 지정되어 있어야 합니다. 이 정보는 디바이스의 전원을 켠 후에 LCD 화면에서 볼 수 있습니다(IP 주소 구성 섹션 참조). 이 포트는 데이터 서브넷 외부에서 웹 기반 UI를 사용할 수 있게 하는 데 사용됩니다.

- Eth3(`10GbE-B`)은 데이터 전송에 사용되며 디바이스 관리에도 사용 가능합니다. 이 연결은 소스 데이터와 동일한 서브넷에 있어야 하거나 필요한 경우 서버에 직접 연결될 수 있습니다.
...
- Eth5(`10Gb SFP+ (5)`)는 일반적으로 데이터 전송에 사용되며 디바이스 관리에도 사용 가능합니다. 이 포트는 10GbE 속도에서만 실행됩니다.

- Eth2(`10-GbE (2)`)는 일반적으로 디바이스 관리에 사용되며 데이터 전송에도 사용 가능합니다. 이 포트는 1GbE 또는 10GbE의 속도로 실행될 수 있습니다.
...
4. 네트워크에 디바이스를 연결하십시오.
    ...
    - 구리선 SFP+ 연결
      ...
      3. 브라우저 `HTTPS://'Your-Eth5-IPAddress'`를 통해 Eth5용으로 구성된 IP 주소에 접속할 수 있으면 다음 단계를 계속하십시오. 그렇지 않으면 Eth2(`10GbE-B` 또는 `1GbE-B`) 포트를 연결하십시오.
...
7. 워크플로우 마법사는 일반적으로 왼쪽에서 오른쪽으로 사용되는 특정 품목에 대한 액세스를 나타냅니다.<br/>
    ...
    인터페이스의 왼쪽 상단에 있는 **워크플로우 관리자**를 사용하여 워크플로우를 다시 열 수 있습니다.
    {:tip}
...
13.	로드가 완료되면 시스템 전원을 정상적으로 끌 수 있습니다. 워크플로우에서 **어플라이언스 종료...**를 클릭하십시오.
