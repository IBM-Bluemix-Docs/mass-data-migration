...
# 將 PureData System for Analytics 資料庫移轉至 {{site.data.keyword.dashdbshort_notm}}
{: #migrate-netezza-databases}

{{site.data.keyword.mdms_full}} 可以用來將大型 IBM PureData™ System for Analytics（採用 Netezza® 技術）資料庫移轉至 {{site.data.keyword.dashdbshort}}。您可以使用本文件作為工具的參照，而這些工具決定要傳送的資料量及匯出方法。
{: shortdesc}
...
- 使用 `CREATE EXTERNAL TABLE` 陳述式。
   - 選取 `FORMAT` = ”Text”
   - 向 {{site.data.keyword.dashdbshort_notm}} 團隊提供在 `LOAD` 處理程序期間用於匯出以供重複使用的 `USING` 子句。
...
如需 PureData System for Analytics 的相關資訊，請參閱 [IBM Netezza 資料庫使用者文件 ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://www.ibm.com/support/knowledgecenter/en/SSULQD_7.2.1/com.ibm.nz.dbu.doc/c_dbuser_plg_overview.html){:new_window}。
